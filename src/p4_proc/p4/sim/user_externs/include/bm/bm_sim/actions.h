...
struct ActionParam {
  ...
  enum {CONST, FIELD, HEADER, ACTION_DATA, REGISTER_REF, REGISTER_GEN,
        ...
        CALCULATION,
        METER_ARRAY, COUNTER_ARRAY, REGISTER_ARRAY, RANDOM,
        EXPRESSION,
        EXTERN_INSTANCE,
        STRING,
...
template <> inline
Random &ActionParam::to<Random &>(
    ActionEngineState *state) const {
  (void) state;
  assert(tag == ActionParam::RANDOM);
  return *(random);
}
...
class ActionPrimitive_ {
 ...
 protected:
  ...
  bool is_primitive(const std::string &primitive_name);
...
class ActionFn :  public NamedP4Object {
 ...
 public:
  ...
  void parameter_push_back_random(Random *random);
