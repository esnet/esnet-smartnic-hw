...
def pkt_accelerator_test(dev, port):
    ...
    pkt_accelerator_config (dev, port, 0)   # mux_out_sel=0 (PHY to APP).
    ...
    pkt_playback_capture_test (dev, _num, _size, _port, False)   # enable_probes=False.
...
def random_traffic_test(dev, port):
    ...
    pkt_accelerator_config (dev, _port, 0)  # mux_out_sel = 0 (PHY to APP).
    ...
    pkt_playback_config (dev, port);  pkt_capture_config (dev, port)
    _bytes = rnd_playback_capture(dev, 100, port)  # 100 packets.
...
def p4_bypass_port_num_test(dev, num, port, num_p4_proc):
    ...
    # configure mux/demux.
    if (port==0):
        dev.bar2.smartnic_regs.smartnic_mux_out_sel[2]._r.value=0  # PF0_VF2 to APP.
        dev.bar2.smartnic_regs.smartnic_demux_out_sel.port1=1      # APP to PF1_VF2.
    else:
        dev.bar2.smartnic_regs.smartnic_mux_out_sel[3]._r.value=0  # PF1_VF2 to APP.
        dev.bar2.smartnic_regs.smartnic_demux_out_sel.port0=1      # APP to PF0_VF2.
...
def phy_path_test(dev, num, port):
    ...
    cmac_loopback_config(dev, port, 1)
...
def probe_to_host_test(dev, num, port):
    ...
    pkt_playback_config (dev, port)  # configure for playback, but not capture.
    ...
    if (port==0):
        names = ['probe_from_pf0_vf2', 'probe_core_to_app0', 'probe_to_app_igr_p4_out0',
                 'probe_to_pf0', 'probe_to_host_0']
    else:
        names = ['probe_from_pf1_vf2', 'probe_core_to_app1', 'probe_to_app_igr_p4_out1',
                 'probe_to_pf1', 'probe_to_host_1']
...
def drops_to_bypass_test(dev, num, port):
    ...
    if (port==0):
        ...
        dev.bar2.smartnic_regs.smartnic_mux_out_sel[3]._r.value=3   # PF0_VF2 to BYPASS DROP
...
def drops_to_host_test(dev, num, size, port):
    ...
    if (port==0):
        dev.bar2.p4_proc_igr.p4_proc.p4_bypass_config.p4_bypass_egr_port_type_0 = 1 # PF0
    else:
        dev.bar2.p4_proc_igr.p4_proc.p4_bypass_config.p4_bypass_egr_port_type_1 = 1 # PF1
