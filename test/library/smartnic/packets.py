...
def pkt_playback_config(dev, port):
    if (port==0): dev.bar2.smartnic_regs.smartnic_mux_out_sel[2]._r.value=0   # PF0_VF2 to APP.
    else:         dev.bar2.smartnic_regs.smartnic_mux_out_sel[3]._r.value=0   # PF1_VF2 to APP.
...
def pkt_capture_config(dev, port):
    ...
    if (port==0): dev.bar2.smartnic_regs.smartnic_demux_out_sel.port0=1   # P0 to PF0_VF2.
    else:         dev.bar2.smartnic_regs.smartnic_demux_out_sel.port1=1   # P1 to PF1_VF2.
...
def pkt_accelerator_config(dev, port, mux_out_sel):
    ...
    cmac_loopback_config(dev, port, 1) # enable=1.
...
def pkt_accelerator_extract(dev, num, exp, port):
    ...
    dev.bar2.smartnic_regs.switch_config.igr_sw_tpause = 0  # assert tpause to igr FIFOs.
...
def pkt_accelerator_flush(dev, port):
    if (port==0): dev.bar2.smartnic_regs.smartnic_mux_out_sel[0]._r.value=3   # PHY0 to DROP.
    else:         dev.bar2.smartnic_regs.smartnic_mux_out_sel[1]._r.value=3   # PHY1 to DROP.
